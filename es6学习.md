...
- 如果结构赋值没有成功配对(  比如没有对应的同名属性 )，变量会是undefined
...
![](C:\Users\Administrator\Pictures\es6\6.JPG)
...
```
{
  //补白  参数：长度，补充内容  在日期、数字字符串处理方面常常用
...
利用 rest 参数，可以向该函数传入任意数目的参数 ，就是取代arguments
...
-  Set 的 key 和 value 都是一样的，都是元素的名称 
...
关于异步编程的方式，常用的定时器、ajax、Promise、Generator、async/await(es7/8)
    ...
    function myIterator(arr){
	    ...
	    return {
         next(){
        	...
        	return index < len ? {value: arr[index++], done: false} : {value: undefined, done: 					 true}
     		  }
	    }
    }
    let arr = [1,4,65,'abc'];
