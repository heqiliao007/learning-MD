...
//then 方法里用箭头函数可以取到this,如果写成then(function(){})则this指向window
```
...
apiready函数相当于入口函数，页面开始加载的时候就会加载它，如果页面开始就要加载接口数据，需要使用到api.ajax，此时api需要加载完成后才能调用，否则报错未定义，所以，初始加载的ajax一般都需要写在apiready中
